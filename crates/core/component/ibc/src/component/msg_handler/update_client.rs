...
use crate::component::{
    ...
    ics02_validation,
    light_client::{ClientKindRead as _, LightClientKind},
    ...
    async fn try_execute<S: StateWrite, AH, HI: HostInterface>(&self, mut state: S) -> Result<()> {
        ...
        match kind {
            LightClientKind::Tendermint => {
                ...
                let trusted_header = verify_tendermint_update::<&S, HI>(
                    &state,
                    &self.client_id,
                    untrusted_header,
                )
...
/// Verify a Tendermint header update against the stored client and
/// consensus states, returning the now-trusted header. Does not write.
pub(crate) async fn verify_tendermint_update<S: StateRead, HI: HostInterface>(
    state: S,
    client_id: &ClientId,
    ...
    client_is_not_expired::<&S, HI>(&state, client_id, &client_state).await?;
